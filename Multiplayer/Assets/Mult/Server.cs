...
public class Server : MonoBehaviour
{
	...
	void OnGUI ()
	{
		...
		if (connected) {
			...
			if (_visible) {
				...
				GUI.Label (new Rect ((Screen.width - 120) / 2, Screen.height / 2 - 35, 120, 30), "Invite: " + Network.connections.Length);
			...
			nname = GUI.TextField (new Rect ((Screen.width - 100) / 2 + 35, Screen.height / 2 - 90, 100, 20), nname);
			...
			if (GUI.Button (new Rect ((Screen.width - 110) / 2, Screen.height / 2, 110, 30), "Invite"))
				// Підключення до сервера
				Network.Connect (ip, Convert.ToInt32 (port));
			
			if (GUI.Button (new Rect ((Screen.width - 110) / 2, Screen.height / 2 + 35, 110, 30), "Create server"))
			...
			if (GUI.Button (new Rect ((Screen.width - 110) / 2, Screen.height / 2 + 70, 110, 30), "Exit"))
				Application.Quit ();
			PlayerPrefab.GetComponent<Client> ().nam = nname;
		}
	}
	...
	void CreatePlayer ()
	{
		...
		_go = (GameObject)Network.Instantiate (PlayerPrefab, transform.position, Quaternion.identity, 1);
